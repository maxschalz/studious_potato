...
class Multi_isotope:
    def __init__(self, concentrations, max_swu=np.inf, n_e=1000, n_s=1000, 
                 process='diffusion', product=np.inf, tails=np.inf, 
                 alpha235=1.6, mstar=350.5):
        """
        ...
        self.check_input(concentrations, process, alpha235, tails, product,
                         max_swu)
    ...
    def check_input(self, concentrations, process, alpha235, tails, 
        product, max_swu):
        """
        ...
        if (max_swu==np.inf
            and tails==np.inf 
            and product==np.inf):
            msg = ("'tails', 'product' and 'max_swu' are set to infinity!\n"
    ...
    def calculate_flows(self):
        ...
        p = self.user_t * e_sum / (1-e_sum)
        t = self.user_p * (1-e_sum) / e_sum
    ...
    def pprint(self, get_results=False):
        ...
        print('  tails              {:11.3f}'.format(self.user_t))
